<template>
  <div >
      <q-toolbar :class="`bg-${event.extendedProps.background} text-white`" style="min-width: 340px;padding-left: 0px;">
        <q-btn flat round color="white" icon="close"   @click="cancel"/>
            <q-toolbar-title >
              <!-- Add New Appointment -->
            </q-toolbar-title>
            <q-btn flat round color="white" icon="delete"   @click="() => {  storeAppointments.deleteAppointment(event.extendedProps.appointmentId);cancel()}"/>
            <q-btn flat round color="white" icon="edit"   @click="storeAppointments.menu[event.extendedProps.appointmentId] = false"/>
          </q-toolbar>
                  <div class="row">
                  <div class="col">
              <q-item clickable @click="()=>{$router.push(`/Patients/${patient.patientId}`);cancel()}" class="q-pt-md">
